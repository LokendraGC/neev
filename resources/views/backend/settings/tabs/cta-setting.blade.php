<div class="tab-pane fade {{ request()->query('tab') == 'cta-section' ? 'active show' : '' }}" id="cta-section" role="tabpanel"
    aria-labelledby="cta-section-tab">
    <div class="">
        <div class="row">
            <div class="col-md-12">
                <div class="mb-3">
                    <label for="cta_background_image" class="form-label">CTA Background Image</label>
                    <div class="input-group open-media-manager" data-bs-toggle="modal" data-bs-target="#exampleModal"
                        style="cursor: pointer;" data-field="cta_background_image" data-select="single">
                        <div class="input-group-prepend">
                            <div class="input-group-text bg-soft-secondary font-weight-medium">
                                Browse</div>
                        </div>
                        <div class="form-control file-amount">Choose File</div>
                    </div>
                    <div class="preview-closer">


                        @if (isset($settings['cta_background_image']) &&
                                ($media = MediaHelper::getModel()->where('id', $settings['cta_background_image'])->first()))
                            <input type="hidden" id="cta_background_image" name="cta_background_image" class="selected-files"
                                value="{{ $settings['cta_background_image'] }}">
                            <div id="cta_background_image_select">
                                <div class="file-preview box sm">
                                    <div
                                        class="d-flex justify-content-between align-items-center mt-2 file-preview-item">
                                        <div
                                            class="align-items-center align-self-stretch d-flex justify-content-center thumb">
                                            <img class="img-fit" src="{{ asset('storage/' . $media->file_name) }}"
                                                alt="image" />
                                        </div>
                                        <div class="col body">
                                            <h6 class="d-flex">
                                                <span
                                                    class="text-truncate title">{{ $media->file_original_name }}</span>
                                                <span class="flex-shrink-0 ext">.{{ $media->extension }}</span>
                                            </h6>
                                            <p>{{ MediaHelper::getKBorMB($media->file_size) }}
                                        </div>
                                        <div class="remove"><button data-id="{{ $media->id }}"
                                                data-slug="cta_background_image" class="btn btn-sm btn-link remove-attachment"
                                                type="button"><i class="bi bi-x-circle"></i></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @else
                            <input type="hidden" id="cta_background_image" name="cta_background_image" class="selected-files"
                                value="" />
                            <div id="cta_background_image_select"></div>
                        @endisset
                </div>

                <span class="form-text text-muted">
                    <small><i>(Recommended Image size: 1860 by 282 pixels)</i></small>
                </span>
            </div>
        </div>
    </div>
</div>
<hr class="mt-0">
<div class="mb-3">
    <label for="cta_title" class="form-label">CTA Title</label>
    
    <input type="text" class="form-control" name="cta_title" id="cta_title" value="{{ isset($settings['cta_title']) ? $settings['cta_title'] : '' }}">
</div>
</div>
