(function($) {
    ...
    ================================ */
      ...
      /* Team member popup (inline) */
      $(".team-popup-open").magnificPopup({
        type: "inline",
        preloader: false,
        removalDelay: 300,
        mainClass: "mfp-fade",
    ...
    /* Media filter form - filter media coverage items (scoped to same section) */
    $('.media-filter-form').on('submit', function(e) {
        e.preventDefault();
        var sector = $(this).find('[name="sector"]').val();
        var company = $(this).find('[name="company"]').val();
        var year = $(this).find('[name="year"]').val();
        var $section = $(this).closest('section');
        var $items = $section.length ? $section.find('.media-coverage-item') : $('.media-coverage-item');
        $items.each(function() {
            var $item = $(this);
            var itemSector = $item.data('sector');
            var itemCompany = $item.data('company');
            var itemYear = $item.data('year');
            var matchSector = !sector || itemSector === sector;
            var matchCompany = !company || itemCompany === company;
            var matchYear = !year || itemYear === year;
            if (matchSector && matchCompany && matchYear) {
...
// Initialize the Text Slider (Vertical)
var textSlider = new Swiper(".vertical-text-slider", {
    direction: "vertical",
    loop: true,
    allowTouchMove: false,
});

// Initialize the Image Slider (Horizontal turned into Fade)
var imageSlider = new Swiper(".main-image-slider", {
    loop: true,
    speed: 2000, // Duration of the fade
    autoplay: { 
        delay: 3000, // Time between slides
        disableOnInteraction: false 
    },
    // --- ADDED FADE EFFECT HERE ---
    effect: 'fade',
    fadeEffect: {
        crossFade: true // This prevents the white/empty background flash between slides
    },
    navigation: {
        nextEl: ".array-next",
        prevEl: ".array-prev",
